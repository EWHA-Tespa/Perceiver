...
class Manager(object):
        ...
        with torch.no_grad():
            with tqdm(total=len(self.val_loader),
                ...
                for batch_idx, (data_a, data_p, label) in enumerate(self.val_loader):
                    ...
                    # ==== compute output ====
                    out_a = self.model.forward_to_embeddings(data_a) #.module.forward_to_embeddings(data_a)
                    out_p = self.model.forward_to_embeddings(data_p) #.module.forward_to_embeddings(data_p)
    ...
    def save_checkpoint(self, optimizers, epoch_idx, save_folder):
        ...
        for name, module in self.model.named_modules(): ##### Jinee ##### self.model.module.named_modules():
        ...
        checkpoint = {
            'model_state_dict': self.model.state_dict(),##### Jinee ##### self.model.module.state_dict(),
            'dataset_history': self.model.datasets, ##### Jinee ##### self.model.module.datasets,
            'dataset2num_classes': self.model.dataset2num_classes, ##### Jinee ##### self.model.module.dataset2num_classes,
    ...
    def load_checkpoint(self, optimizers, resume_from_epoch, save_folder):
        ...
        if resume_from_epoch > 0:
            ...
            curr_model_state_dict = self.model.state_dict() ##### Jinee ##### self.model.module.state_dict()
